  ...
  <div class="container mx-auto py-8 px-4 max-w-7xl">
    ...
    >
      <div class="relative inline-block">
        <h1
          class="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-extrabold text-white tracking-tight"
        >
          Event
          <span
            class="block mt-2 text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 via-violet-400 to-purple-300"
          >
            Schedule
          </span>
        </h1>
        <div
          class="absolute -inset-x-6 -inset-y-4 bg-gradient-to-r from-indigo-500/10 via-violet-500/10 to-purple-500/10 blur-3xl animate-pulse-slow"
        ></div>
      </div>

      <p
        class="mt-6 text-lg sm:text-xl text-slate-300/90 max-w-2xl mx-auto px-4 leading-relaxed font-light"
      >
        Stay informed with our comprehensive event schedule. Track upcoming
        events, PK windows, and special activities to make the most of your
        adventure.
      </p>

      <div
        class="mt-12 h-px w-32 mx-auto bg-gradient-to-r from-transparent via-slate-700 to-transparent"
      ></div>
    </div>
    ...
    <div class="mb-8">
      <div class="bg-gray-800/50 backdrop-blur-sm rounded-lg overflow-hidden">
        <div class="relative p-6">
          <div
            class="absolute inset-0 bg-gradient-to-br from-red-600/5 via-pink-600/5 to-purple-600/5"
          ></div>
          <div class="relative">
            <div class="flex items-center justify-between mb-4">
              <h3
                class="text-xl font-semibold text-slate-100 flex items-center"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-6 w-6 mr-2 text-red-400"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              <div
                class="bg-gray-900/50 rounded-lg p-4 hover:bg-gray-900/70 transition-colors"
              >
                <div class="text-sm text-slate-400">Cycle Time</div>
                <div class="text-lg font-semibold text-slate-200">
                  {{ pkSettings.settings.timeout }}
                </div>
              </div>
              <div
                class="bg-gray-900/50 rounded-lg p-4 hover:bg-gray-900/70 transition-colors"
              ...
              >
                ...
                <div class="text-lg font-semibold text-emerald-400">
                  {{ pkSettings.settings.expMultiplier }}
                </div>
              </div>
              <div
                class="bg-gray-900/50 rounded-lg p-4 hover:bg-gray-900/70 transition-colors"
              >
                ...
                <div class="text-lg font-semibold text-amber-400">
                  {{ pkSettings.settings.dropMultiplier }}
                </div>
              </div>
            </div>
            ...
            <div class="bg-gray-900/30 rounded-lg p-4">
              <div class="flex justify-between items-center">
                ...
                <div
                  ...
                  "
                >
                  {{ getCountdown(pkSettings.next) }}
                </div>
              </div>
              <div class="mt-2 text-sm text-slate-500 text-center bg-gray-800/30 rounded-lg p-3 text-white font-bold uppercase">
                {{
                  pkSettings.isActive
                    ? "PK Mode is currently active"
                    : ""
                }}
              </div>
            </div>
    ...
    <div v-else>
      ...
      <div class="space-y-8">
        <!-- Next Events Section -->
        <div>
          <h3 class="text-2xl font-semibold text-slate-100 mb-6">
            Next Events
          </h3>
              ...
              <div class="relative p-6 space-y-4">
                ...
                <div class="text-sm space-y-1.5 bg-gray-800/30 rounded-lg p-3">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="text-slate-300">{{
                      event.requirements.fee
                    }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-slate-400">Capacity:</span>
                    <span class="text-slate-300">{{
                      event.requirements.players
                    }}</span>
                  </div>
                  <div
                    v-if="event.requirements.lives"
                    class="flex justify-between"
                  >
                    <span class="text-slate-400">Lives:</span>
                    <span class="text-slate-300">{{
                      event.requirements.lives
                    }}</span>
                  </div>
                  <div
                    v-if="event.requirements.duration"
                    class="flex justify-between"
                  >
                    <span class="text-slate-400">Duration:</span>
                    <span class="text-slate-300">{{
                      event.requirements.duration
                    }}</span>
                  </div>
                </div>
          ...
          >
            <span class="text-slate-200 font-semibold">
              {{
                showAllEvents
                  ? "Show Less"
                  : `Show More (${remainingEvents.length} events)`
              }}
            </span>
          </button>
        </div>
  ...
  setup() {
    ...
    const remainingEvents = computed(() => {
      if (activeFilter.value === "all") {
        return events.value
          .filter(
            (event) => !nextEvents.value.find((e) => e.next === event.next)
          )
    ...
    // Updated countdown function using moment.js
    const getCountdown = (nextTime) => {
      const now = moment();
      const target = moment(nextTime);
    ...
    const getEventTypeGradient = (type) =>
      ({
        siege:
          "bg-gradient-to-br from-indigo-600/10 via-violet-600/10 to-purple-600/10",
        ...
        secretroom:
          "bg-gradient-to-br from-rose-600/10 via-pink-600/10 to-fuchsia-600/10",
      }[type] || "");

    const getEventTypeBorder = (type) =>
      ({
        siege:
          "bg-gradient-to-br from-indigo-500/50 via-violet-500/30 to-purple-500/50",
        ...
        secretroom:
          "bg-gradient-to-br from-rose-500/50 via-pink-500/30 to-fuchsia-500/50",
      }[type] || "");
