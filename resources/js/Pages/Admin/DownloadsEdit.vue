...
const submitForm = () => {
  ...
  axios
    .post(`/administrator-panel/downloads/edit/${props.download.id}`, form.value)
    .then((res) => {
      isSubmitting.value = false;

      if (res.status === 200) {
        new Notyf().success("Download link updated successfully.");
...
watchEffect(() => {
  if (props.download) {
    form.value.title = props.download.title;
    form.value.type = props.download.type;
    form.value.link = props.download.link;
    form.value.description = props.download.description;
  }
});
</script>
